...
//   const isAdmin = user?.user?.role === 'admin' || session?.user?.role === 'admin';
...
//   // If admin is logged in, show minimal header with only logo and sign out
//   if (isAdmin) {
//     return (
//       <header
//         className={`fixed top-0 py-1 z-50 w-full bg-transparent transition-all ${sticky ? "shadow-lg bg-white dark:bg-darklight" : "shadow-none"
...
//                 className="absolute w-fit text-sm font-medium text-center z-10 invisible group-hover:visible transition-opacity duration-200 bg-primary text-white py-1 px-3 min-w-28 rounded-md shadow-2xl top-full left-1/2 transform -translate-x-1/2 mt-3"
//               >
//                 {user?.user?.name || session?.user?.name} (Admin)
//               </p>
//             </div>
//             <button
//               onClick={() => handleSignOut()}
//               className='bg-primary text-sm hover:bg-orange-600 text-white px-4 py-3.5 border border-primary duration-500 leading-none rounded-lg font-medium text-nowrap cursor-pointer'
//             >
//               Sign Out
//             </button>
//           </div>
...
//   // Regular header for non-admin users
//   return (
//     <header
//       className={`fixed top-0 py-1 z-50 w-full bg-transparent transition-all ${sticky ? "shadow-lg bg-white dark:bg-darklight" : "shadow-none"
//         }`}
